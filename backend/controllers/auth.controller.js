...
const registerUser = async (req, res) => {
    try {
        ...
        const verificationToken = jwt.sign(
            {userId: newUser._id, porpuse: 'email-verification'},
    ...
    try {
        ...
        if (!user.isEmailVerified) {
            ...
            if(existingVerification && existingVerification.expiresAt > new Date()) {
                ...
                const verificationToken = jwt.sign(
                    { userId: user._id, porpuse: 'email-verification' },
                    process.env.JWT_SECRET,
                    { expiresIn: '1h' }
                );
...
const verifyEmail = async (req, res) => {
    try {
        ...
        const { userId, porpuse } = payload;

        if (porpuse !== 'email-verification') {
            return res.status(401).json({ message: 'Unauthorized'});
        }
...
const resetPasswordRequest = async(req, res) => {
    try {
        ...
        const resetPasswordToken = jwt.sign(
            { userId: user._id, porpuse: 'reset-password' },
            process.env.JWT_SECRET,
            { expiresIn: '15m' }
        )
...
const verifyResetPasswordTokenAndResetPassword = async (req , res ) => {
    try {
        ...
        const isTokenEpired = verification.expiresAt < new Date();
